...
@Component({
  selector: 'app-servicios-r',
  templateUrl: './servicios-r.component.html',
  styleUrls: ['./servicios-r.component.css']
})
export class ServiciosRComponent implements OnInit {
  ...
  buscar(ejemplo){
    ...
    if(this.subcategoriaSelected>=0){
      ejemplo.idFichaClinica.idTipoProducto={idTipoProducto:this.subcategoriaSelected}
    }
    let inicio=(this.pagina_actual-1)*this.limite;
    
    this.service=this.resServ.listarServicios('0','0',this.orderBy,ejemplo).subscribe(
      (response)=>{
